...
describe('KmlRiskPointSource', () => {
    let source;
    beforeEach(() => {
        source = new kml_risk_point_source_service_1.KmlRiskPointSource();
    });
    afterEach(() => {
    ...
    describe('parseKml', () => {
        ...
        it('keeps mock statistics formula keyed on cluster rank', () => {
            ...
            expect(points[0].riskLevel).toBe(enums_1.RiskLevel.CRITICAL);
            expect(points[1].accidentCount).toBe(417);
            expect(points[1].riskLevel).toBe(enums_1.RiskLevel.CRITICAL);
        });
    ...
    describe('fetchRiskPoints', () => {
        it('returns null when the request fails with non-ok status', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(kml_risk_point_source_service_1.GOOGLE_MAPS_KML_URL, expect.objectContaining({ signal: expect.anything() }));
        });
...
describe('MockDataService fallback', () => {
    it('keeps seeded mock points when the KML source returns null', async () => {
        ...
        const service = new mock_data_service_1.MockDataService(source);
    ...
    it('replaces seeded points when the KML source returns points', async () => {
        const kmlSource = new kml_risk_point_source_service_1.KmlRiskPointSource();
        const kmlPoints = [
            {
                riskPointId: 'RP-001',
                clusterRank: 1,
        ...
        const service = new mock_data_service_1.MockDataService(source);
        await service.onModuleInit();
        expect(service.riskPoints).toEqual((0, risk_point_pdf_details_1.withPdfDetails)(kmlPoints));
...
//# sourceMappingURL=kml-risk-point-source.service.spec.js.map